...
3. **Compositor-only y-axis transition via Web Animations API** (`Smile.tsx` `useLayoutEffect` on yRange change). Re-renders content at the NEW range immediately, then runs WAAPI animation on the inner chart group via `element.animate(...)`. Interpolation runs on the GPU compositor — zero React re-renders, zero main-thread style/layout/paint per animation frame. `transform-origin: 0 0` with `transform-box: view-box` (load-bearing for **correctness**, not perf: it pins origin 0 0 to the fixed SVG viewport, which the ty/sy math assumes; under `fill-box` the origin tracks the group's moving bbox corner and mis-renders the rescale. A controlled A/B incl. a churning bbox showed no layout/style-recalc difference between the two). Animation handle stored in ref so a new yRange change can cancel an in-flight one. Respects `prefers-reduced-motion: reduce` via `duration: 0`.
