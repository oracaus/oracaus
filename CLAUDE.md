...
Vol-shock multiplier amplifies σ_spot and OU diffusion × 5 for a 10-second burst. URL params: `?seed=<int>`, `?mode=recording`, `?rate=<n>` (initial feed tick rate in Hz; clamped to [1, 1000], default 50 — lets a capture open straight at e.g. 500 Hz and survive the reload Lighthouse forces, since the toolbar rate is React state that resets on reload while the URL param does not).
...
3. **Compositor y-axis transition via Web Animations API** (`Smile.tsx` `useLayoutEffect` on yRange change). The chart is split into a static base SVG (axes/grid/labels) and an **HTML-wrapped interior layer** (dots/curve/cursor). Re-renders content at the NEW range immediately, then runs a WAAPI `transform` animation on the **HTML wrapper** (`interiorLayerRef`) via `element.animate(...)`. The HTML element is load-bearing: Chrome composites transform animations on HTML/replaced elements but **not on SVG child elements** — a `<g>` transform reports `compositeFailed` and falls back to per-frame main-thread layout (confirmed in a trace: 18× `compositeFailed=1024`, ~60 layouts/sec spikes during a shock). On the HTML wrapper the interpolation runs on the compositor: zero React re-renders, zero per-frame style/layout/paint. The wrapper sits `inset-0` over the base SVG with `transform-origin: 0 0`, so its border-box origin coincides with the view-box origin and the ty/sy maths are unchanged from the prior SVG-group version. `will-change: transform` is set for the transition's duration and cleared on finish (no permanently-promoted 200-node layer at rest). Animation handle stored in a ref so a new yRange change cancels an in-flight one. Respects `prefers-reduced-motion: reduce` via `duration: 0`. (Earlier versions animated the SVG `<g>` and assumed it composited because it appeared in DevTools' Animations panel; a non-composited WAAPI animation appears there too — the `compositeFailed` flag is the real check.)
